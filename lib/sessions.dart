...
class _SessionsPageState extends State<SessionsPage> {
  ...
  _p2pEventHandler(P2PMessage event) {
    switch (event){
     ...
     case EventClientConnect(timestamp: int timestamp, name: String name, keyFingerprint: String keyFingerint, socket: Socket socket):
        sessions.addAll(<String,Session>{
          keyFingerprint: Session(name: name, keyFingerprint: keyFingerprint, socket: socket),
        );
     case EventClientDisconnect(timestamp: int timestamp, publicKey: String publicKey):
     default:;
    }
