...
// Note: when msgdec.mimeType == 'application/text', the field, Filename
// and Filename length must be omitted. (NYI)
...
class _P2PSocket {
  ...
  String _eType(){
    switch(endpointType) {
      case _EndpointType.client: return 'client';
      case _EndpointType.server: return 'server';
    }
  }

  String _mKey(){
    return endpointFingerprint + '-' + _eType(); 
  }
...
//  ......    | data
    try {
      ...
      // timestamp
      int timestamp = P2PUtils.readU64Int(buf);
      print('message timestamp ${timestamp}');
  
      Uint8List listMimeType = P2PUtils.read32Field(buf);
      Uint8List listData = P2PUtils.read32Field(buf);
 ...
 //////// and kill the connection.
  void _parseMessage(Uint8List data) async {
    try {
      _buf.addAll(data);
      if (_buf.length + data.length > P2PSettings.maxWindowSize) {
        print('exceeded max window size $endpointFingerprint');
        socket.destroy();
        return;
      }
      ...
      // Encrypted data
      Uint8List listEncrypted = P2PUtils.read32Field(_buf);
      Uint8List listSignature = P2PUtils.read32Field(_buf);
...
//      print('signature ${listSignature}');
  ...
  void _parseHandshake(Uint8List data) {
    try {
      ...
      int timestamp = P2PUtils.readU64Int(_buf);
      HandshakeBlob.add(P2PUtils.uint64ToList(timestamp));

      // dstKeyFingerprint
      Uint8List listKeyFingerprint = P2PUtils.read32Field(_buf);
      ...
      Uint8List listPkey = P2PUtils.read32Field(_buf);
      endpointPkey = String.fromCharCodes(listPkey);
      HandshakeBlob.add(listPkey);

      // Signature
      Uint8List listSignature = P2PUtils.read32Field(_buf);
      print('parsing successful $timestamp');
      print('Handshakeblob len $HandshakeBlob.length');
      print(HandshakeBlob.toBytes());

      _verifyBytes(String.fromCharCodes(listSignature), HandshakeBlob.toBytes()).then((isValid){
        if (!isValid) {
        ...
        OpenPGP.getPublicKeyMetadata(endpointPkey).then((PublicKeyMetadata meta) {  
          ...
          activeClients.addAll(<String, _P2PSocket>{_mKey():this});
...
//  .......   | PGP signature of the above first 6 fields
    ...
    socket.flush();
    print('handshake length ${b.length} sent');
  }
...
class P2PService {
  ...
  Map<String, _P2PSocket> _activeEndpoints = <String, _P2PSocket>{};
  ...
  _onClient(Socket client) {
    ...
    _clients.add(_P2PSocket.endpointClient(
      ...
      getPass: getPass,
      activeClients: _activeEndpoints,
  ...
  Future<bool> destroy(String keyFingerprint) async {
    ...
    p2p = _activeEndpoints[keyFingerprint + '-client'];
    if (p2p != null)
      return await p2p!.destroy();

    p2p = _activeEndpoints[keyFingerprint + '-server'];
    if (p2p != null)
      return await p2p!.destroy();
  ...
  Future<bool> SendMessageText(String fingerprint, String message) async {
    ...
    p2p = _activeEndpoints[fingerprint + '-client'];
    if (p2p != null)
      return await p2p.SendMessageText(message);
    p2p = _activeEndpoints[fingerprint + '-server'];
    if (p2p != null)
      return await p2p.SendMessageText(message);

    print('no such active client $fingerprint');
    return Future.value(true);
  }
}
